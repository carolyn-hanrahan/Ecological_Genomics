...
COV <- as.matrix(read.table("allRS_poly_hw1.cov")) # read in the genetic covariance matrix
...
plot(PCA$vectors[,1:2],
     ...
     xlab="PC1",ylab="PC2", 
     main="Genetic PCA (PC1 vs. PC2)")


plot(PCA$vectors[,2:3],
     col=as.factor(pops[,2]),
     xlab="PC2",ylab="PC3", 
     main="Genetic PCA (PC2 vs. PC3)") 
...
q <- read.table("allRS_poly_hw1.admix.3.Q", sep=" ", header=F)
